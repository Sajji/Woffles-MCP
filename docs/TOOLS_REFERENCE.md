...
Find catalog Column assets by metadata keyword search can't filter on. All supplied filters are AND-combined into one Knowledge Graph GraphQL query. Requires the Knowledge Graph GraphQL API to be enabled.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `description_contains` | No | Description attribute contains this text |
| `data_type_contains` | No | Data Type attribute contains this text (e.g. `VARCHAR`) |
| `steward_name_contains` | No | Has a Data Steward whose full name contains this text |
| `related_to_name` | No | Related (either direction) to an asset whose name contains this text |
| `related_to_types` | No | Type names the related asset may have (default: Business Term, Business Rule, Data Element, Data Attribute) |
| `domain_id` | No | Restrict to a domain UUID |
| `asset_type_public_id` | No | Asset type public ID to search (default: `Column`) |
| `limit` / `offset` | No | Pagination (default 50 / 0) |

At least one of the four content filters is required. String matching is case-sensitive `contains`.

---

## Assessments — Typed Editing

### edit_assessment *(write)*

Edit a conducted assessment via typed operations applied as a **single atomic PATCH** (all-or-nothing). Prefer this over the deprecated `update_assessment`.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `assessment_id` | No* | UUID of the assessment (*or `assessment_name`) |
| `assessment_name` | No* | Exact assessment name (ambiguity returns candidates) |
| `operations` | Yes | Ordered list of typed ops (below) |
| `confirm` | No | `false` (default) = preview; `true` = apply |

**Operations:**
- `set_answer` — `question_id` + `value` (TEXT/HTML/EXPRESSION/NUMBER/BOOLEAN/DATE) or `items` (ITEMS); `answer_type` required for not-yet-answered questions; optional `comments`. ASSETS/USERORGROUPS/ATTACHMENTS are not supported.
- `set_status` — `value`: DRAFT, SUBMITTED, or OBSOLETE
- `set_name` — `value`: the new name
- `set_owner` — `owner_id`: user UUID
- `set_assignees` — `assignees`: full replacement list of `{type: USER|GROUP, id}`
...
List deployed workflow definitions (approval, review, onboarding processes).

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `name` | No | Filter by name (partial match) |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### find_workflow_tasks

List open workflow tasks (the human to-do items generated by running workflows).

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `title` | No | Filter by task title |
| `business_item_id` | No | UUID of the related asset/domain/community |
| `user_id` | No | Tasks assigned to / claimable by this user |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### start_workflow_instance *(write)*

Start a workflow against one or more business items. **The preview surfaces the start-form fields** so required `form_properties` can be supplied before confirming. Starting a workflow can create tasks and send notifications to real users.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `workflow_definition_id` | No* | UUID of the definition (*or `workflow_definition_name`) |
| `workflow_definition_name` | No* | Exact definition name |
| `business_item_ids` | No | UUIDs of the target resources (omit for global workflows) |
| `business_item_type` | No | `ASSET` (default), `DOMAIN`, or `COMMUNITY` |
| `form_properties` | No | Start-form values keyed by form property ID |
| `send_notification` | No | Default: true |
| `confirm` | No | `false` (default) = preview; `true` = start |

### complete_workflow_task *(write)*

Complete an open workflow task (e.g. approve/reject a review step). The preview surfaces the task-form fields.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `task_id` | Yes | UUID of the task (from find_workflow_tasks) |
| `form_properties` | No | Task-form values keyed by form property ID |
| `confirm` | No | `false` (default) = preview; `true` = complete |
...
Find comments on assets/domains/communities.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `base_resource_id` | No | UUID of the commented resource |
| `user_id` | No | Author UUID |
| `parent_id` | No | Comment UUID — returns its replies |
| `root_only` | No | Only root comments |
| `resolved` | No | Filter by resolved state |
| `sort_order` | No | `ASC` / `DESC` (default DESC on creation date) |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### add_comment *(write)*

Add a comment to a resource, or reply to a comment. Markdown is auto-converted to HTML.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `content` | Yes | Comment text (Markdown or HTML) |
| `resource_id` | No* | UUID of the resource (new root comment) |
| `resource_type` | No | `Asset` (default), `Domain`, `Community` |
| `parent_id` | No* | Comment UUID to reply to |
...
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `context_id` | No | UUID of the resource whose activities to fetch |
| `performed_by_user_id` | No | Acting user UUID |
| `categories` | No | e.g. `ATTRIBUTE`, `RELATION`, `STATUS`, `WORKFLOW`, `RESPONSIBILITY`, `TAGS` |
| `resource_discriminators` | No | e.g. `Asset`, `Domain`, `Community` |
| `start_date` / `end_date` | No | ISO 8601 date range |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### find_ratings

User ratings (1–5 stars + review text) on assets, with a computed average when filtered to one asset.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `asset_id` | No | Asset UUID |
| `user_id` | No | Author UUID |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### get_asset_view_stats

Navigation statistics: most-viewed assets (all users) or recently-viewed (service account).
...
## Tags, Mappings, Users & Complex Relations

### find_tags

List tags, optionally with the assets carrying each tag.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `name` | No | Filter by tag name |
| `include_assets` | No | Also fetch assets per tag (default false) |
| `assets_per_tag` | No | Cap per tag (default 25) |
| `limit` / `offset` | No | Pagination (default 100 / 0) |

### find_mappings / add_mapping *(write)* / remove_mapping *(write)*

External-system mappings: links between an external entity ID and a Collibra resource. `add_mapping` is idempotent by (system, entity) pair; `remove_mapping` uses preview/confirm and accepts a mapping UUID or the (system, entity) pair.

Key parameters: `external_system_id`, `external_entity_id`, `mapped_resource_id` (+ optional `description`, `external_entity_url`).

### find_users

Resolve users by name fragment (username/first/last/email). Feeds `set_responsibility`, `set_owner`, `set_assignees`, and `performed_by_user_id`.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `name` | No | Name fragment |
| `name_search_fields` | No | `USERNAME`, `FIRSTNAME`, `LASTNAME`, `EMAIL` |
| `group_id` | No | Only members of this user group |
| `only_enabled` | No | Only active users |
| `limit` / `offset` | No | Pagination (default 50 / 0) |

### find_complex_relations

Multi-leg relations connecting 3+ assets (legs with role/co-role + attributes).

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `asset_id` | No | Only complex relations involving this asset |
| `type_public_ids` | No | Complex relation type public IDs |
| `limit` / `offset` | No | Pagination (default 100 / 0) |
...
### export_output_module

Run an Output Module TableViewConfig/ViewConfig JSON query and return the results inline — Collibra's report-style query engine (one call joins assets with attributes, relations, domain, status).

| Parameter | Required | Description |
|-----------|----------|-------------|
| `instance_name` | Yes | Collibra instance name |
| `view_config` | Yes | Object with a top-level `TableViewConfig` or `ViewConfig` key |
| `validate` | No | Server-side syntax validation before executing (default false) |
...
List the available workflow guides (no Collibra connection needed). Embedded skills: `collibra/operating-model`, `collibra/cross-instance-migration`, `collibra/assessments`, `collibra/discovery-lineage`. Add your own via the `skillsDir` config key or `COLLIBRA_MCP_SKILLS_DIR` (layout: `<dir>/<namespace>/<name>/SKILL.md`); same-name external skills replace embedded ones.

### load_collibra_skill

Load a skill guide by `namespace` + `name` (optional `reference` file). Read it fully before attempting the corresponding workflow.
...
List Context Specifications — definitions of how to extract governed metadata starting from an asset, shaped for a target system (Snowflake, Databricks, or custom AI-agent shapes). Filter by `asset_id` or `asset_type_public_id`. Requires the Semantic Blueprint capability.

### get_context_specification

Retrieve one Context Specification by UUID, including its full `mappingYaml`.

**Related:** `get_asset_by_id` accepts `context_specification_id` to generate the asset's structured YAML context (`POST /rest/contextEngine/v1/contexts/generate`), and `include_breadcrumb` to return the community/domain path.
...
- **Streamable HTTP** — start with `--http[=port]`, `COLLIBRA_MCP_HTTP_PORT`, or `"http": {"enabled": true, "port": 3399}` in config.json. Stateless (one server per request), binds to `127.0.0.1` only. Requests use the server-configured credentials — do not expose beyond the local machine.
- **`validateOutput: true`** — warn-only ajv validation of every tool's `structuredContent` against its `outputSchema` (logged to stderr).
- **Markdown→HTML** — write tools (`create_asset`, `bulk_create_assets`, `add_business_term`, `edit_asset`, `add_comment`) automatically convert Markdown values targeting RICH_TEXT attributes into HTML.
